...
> 솔루션이 지도 위 제품 경로를 밝히는 렌즈, 요청의 일생을 정거장별 자막으로
> 따라가는 재생, 백지 회상 시험, 검증한 영역이 시간이 지나면 다시 흐려지는
> 간격 반복 재안개, 진도 리포트 내보내기), 솔루션이 어떤 제품들로 구성되는지
> 보여주는
...
(`/?product=<id>`, `/?lens=<id>`, `/?mode=replay&stop=<n>`, `/?mode=recall&area=<layer>`,
